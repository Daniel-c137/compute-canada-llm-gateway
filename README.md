...
**Why do I get an error pulling / downloading the model?**
...
| **Exit 4** before `sbatch` | Preflight found `**speculators_config`** with a **Hub-only verifier** that would break offline on the GPU node. Clean or re-download weights (see next FAQ). |
...
**vLLM logs a different Hugging Face repo, or** `LocalEntryNotFoundError` **offline**
